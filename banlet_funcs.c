...
int *find_linebreaks(char *msg, int *nbreaks){
        ...
        *nbreaks = count;
       int *breaks = malloc(count * sizeof(int));
       int pos = 0;
        int i=0;
	for (; msg[i] != '\0'; i++) {
        if (msg[i] == '\n') {
            breaks[pos++] = i; // Start of the next line
        }
...
void print_fontified(char *msg, font_t *font){
    ...
    for (int i = 0; i < nbreaks - 1; i++) {
        int line_length = breaks[i + 1] - breaks[i] - 1; // -1 to exclude the newline character
        print_fontified_oneline(msg + breaks[i], font, line_length);
    }
    free(breaks);
}
